function [IOut,output] = denoiseImageKSVD(Image,train_batch, sigma,K,varargin)
%==========================================================================
%   P E R F O R M   D I C T I O N A R Y  L E A R N I N G
%==========================================================================
...
maxNumBlocksToTrainOn = train_batch*100;
displayFlag = 1;
...
%if(prod([NN1,NN2]-bb+1)> maxNumBlocksToTrainOn/10)
if(prod([NN1,NN2]-bb+1)> maxNumBlocksToTrainOn/train_batch)
    blkMatrix = zeros(bb^2,maxNumBlocksToTrainOn/train_batch);
    for j = 1:train_batch
        randPermutation =  randperm(prod([NN1,NN2]-bb+1));
        selectedBlocks = randPermutation(1:maxNumBlocksToTrainOn/train_batch);

        for i = 1:maxNumBlocksToTrainOn/train_batch
            [row,col] = ind2sub(size(Image(:,:,j))-bb+1,selectedBlocks(i));
            currBlock = Image(row:row+bb-1,col:col+bb-1,j);
            blkMatrix(:,(j-1)*(maxNumBlocksToTrainOn/train_batch) + i) = currBlock(:);
        end
    end
...
%    end
    ...
    for j = 1:train_batch
        if j == 1
            blkMatrix = im2col(Image(:,:,j),[bb,bb],'sliding');
        else 
            blkMatrix = [blkMatrix' ;im2col(Image(:,:,j),[bb,bb],'sliding')']';
        end
    end
end
