...
%==========================================================================
Z = zeros(train_batch,28,28);
for i = 1:train_batch
    [IoutGlobal,output] = denoiseImageGlobal(IMin(:,:,i), sigma,K);
    for j = 1:28
        for k = 1:28
            Z(i,j,k) = IoutGlobal(j,k);
        end
    end
end

Z = reshape(Z,1,train_batch*28*28);
csvwrite('Z.csv',Z)
